...
## Purpose
This contract defines the minimum PaperImplementation V1 flow that T-109 must make repeatable. It is a route-level replay contract backed by service-level fixture helpers. It does not create new authority objects and does not bypass existing authority writers.
...
| 13 | `POST /trace-manifests` for `run_evidence_unit:<id>` and T-104 `collect` or `POST /run-monitor-intakes` | Target-specific run evidence trace, external job ref/hash, run status | Trusted `RunEvidenceUnit` | Final evidence requires `run_evidence_unit_id` and complete run-evidence trace | Orphan callback is untrusted; missing target trace blocks final evidence. |
| 14 | `POST /trace-manifests` for `result_interpretation_packet:<id>` and `POST /result-interpretation-packets` | Trusted run evidence, metrics, failed/inconclusive run accounting | `ResultInterpretationPacket` | Experiment lineage required for result interpretation packet | Result summary bounds claim implications; failed runs retained. |
...
| 18 | `POST /implementation-dossiers/:dossier_id/writing-entry-packets` | Ready dossier, projection policy | `WritingEntryPacket` projection | Projection only; no writing-module authority mutation | This is the V1 writing boundary. |
