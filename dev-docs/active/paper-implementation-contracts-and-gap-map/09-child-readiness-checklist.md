...
| T-096 WorkOrder bridge | Admitted validation cycle / experiment plan, experiment-foundation contracts, trace guidance | `ResearchWorkOrder`, broker/harness, monitor intake, ledger writer, `RunEvidenceUnit` | WorkOrder and ingestion services through future `StateWriter` | WorkOrder admission, reproducibility, run policy, monitor trust, failed-run retention | WorkOrder id, run type/status, dataset/code/config refs, trace refs queryable | Tests for missing work order, missing dataset/code/config, failed-run omission, AutoTune primary misuse |
| T-098 result/claim/dossier | `RunEvidenceUnit`, validation reports, trace manifests, motive refs | `ResultInterpretationPacket`, `ClaimCandidate`, `ClaimTracePacket` usage, `ImplementationDossier`, packet projection, feedback candidates | Result/claim/dossier service through `StateWriter` | result interpretation, claim boundary, claim trace, dossier readiness, export confirmation, feedback no-overwrite | Dossier status, claim trace refs, failed-run count, readiness gate refs queryable | Tests for overclaim, missing failed runs, missing trace, strong-claim confirmation, stale packet, feedback triggers |
