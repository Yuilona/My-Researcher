...
Acceptance:
- [ ] Slice schema preserves upstream refs and constraint profile version.
...
Acceptance:
- [ ] Selection fails when the intake readiness is not `ready_for_slice`.
- [ ] Selection fails when selected option exceeds claim ceiling or violates non-goals.
...
Acceptance:
- [ ] TopicQuestion package can consume selected slice refs and boundary fields.
