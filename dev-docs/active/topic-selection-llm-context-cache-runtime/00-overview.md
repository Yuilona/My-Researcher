...
- v1c production-depth remains passed locally, and DashScope live provider acceptance now has N2/N4/N6 slot evidence. OpenAI live acceptance is not yet closed: focused diagnostics show provider attempts occur, token-budget gates are `within_budget`, and response reuse remains null, but the local OpenAI channel currently blocks at the provider request/network layer. Final dual-provider live acceptance requires rerunning the gated OpenAI N2/N4/N6 canaries once that provider channel is stable.
