...
| v1c N2 | `topic-selection.v1c.generate-promotion-support.v1` | `n2_bounded_micro_debate.promotion_supporter_repair` | implemented L1-L5 local through `TopicSelectionV1cN2BoundedDebateRuntimeService -> AgentOrchestrator`; prior supporter/critic artifact hashes are dynamic material and admission-checked; `TopicSelectionProviderCanaryService` covers this production runtime slot for local/live provider-required semantics | support-only repair before final synthesis; repair actions cannot override deterministic N3 gate or allowed refs | `v1c_n2_bounded_promotion_support` + critic finding ref/hash as dynamic material; owner v1c bounded-debate runtime/admission services; compression MUST preserve prior role hashes, critic findings, allowed refs, risk/recheck facts | `topic-selection.v1c.promotion-support.bounded-micro-debate.v1`; `TopicSelectionV1cBoundedMicroDebateRoleOrFinal@v1`; prior-role hash drift blocks admission; L4 provider canary uses slot-derived strict schema and L5 compression adversarial coverage passed |
| v1c N2 | `topic-selection.v1c.generate-promotion-support.v1` | `n2_bounded_micro_debate.synthesizer_final` | implemented L1-L5 local through `TopicSelectionV1cN2BoundedDebateRuntimeService -> AgentOrchestrator`; admission recomputes expected runtime identity through the runtime service and emits a sealed admission identity/hash; admitted final output feeds `TopicSelectionV1cPromotionGateService.createPromotionDecisionSupportFromVerifiedRuntimeDraft` before explicit N3 gate creation; `TopicSelectionProviderCanaryService` covers this production runtime slot for local/live provider-required semantics; included in `pnpm topic-selection:v1c-runtime-stress` | final advisory semantic layer before deterministic N3 promotion gate; only admitted support draft/dossier input, never gate authority; support replay key is bound to the admitted runtime identity hash | `v1c_n2_bounded_promotion_support` + role-output refs/hashes as dynamic material; owner v1c bounded-debate runtime/admission services; compression MUST preserve critic findings, critic resolution map, readiness coverage items, allowed refs, risk/recheck obligations, and source hashes | `topic-selection.v1c.promotion-support.bounded-micro-debate.v1`; `TopicSelectionV1cBoundedMicroDebateRoleOrFinal@v1`; final prompt drift blocks with `N2_BOUNDED_DEBATE_ARTIFACT_PROMPT_DRIFT`; prompt-cache read-through and no-N3-bypass smoke passed; L4 provider canary uses slot-derived strict schema and L5 compression adversarial coverage passed |
...
- v1c provider canary slots: N2 bounded micro-debate supporter draft, reviewer critic, supporter repair, synthesizer final; N4 delegated promotion decision candidate; N6 downstream feedback normalization. These are production runtime slot canaries through `TopicSelectionProviderCanaryService`; no v1c provider-canary-only path is allowed.
