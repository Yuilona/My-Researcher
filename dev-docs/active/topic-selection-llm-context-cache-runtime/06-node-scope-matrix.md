...
|---|---|---|
| `n6_question_candidate_draft.regeneration_after_n7_loopback` | implemented L1/L2 | consumes exactly one ref-backed `v1b_n7_to_n6_failed_trial_loopback_context` artifact; projection, selected-slice lineage, old N6 handoff hash, failed-trial synthesis, exhausted candidates, failure reasons, regeneration hints, and optional N8 feedback participate in runtime/prompt identity; orphan, prompt drift, source drift, and unknown exhausted refs block before authority writes |
| `n6_question_candidate_draft.regeneration_after_n6_gate_failure` | implemented L1/L2 | consumes exactly one ref-backed `v1b_n6_gate_failure_retry_context` artifact; failed draft identity, failed draft prompt packet hash, failed draft source-hash bundle, blocked-candidate context, failure reasons, regeneration hints, selected slice identity, and N5 handoff hash participate in runtime/prompt identity; orphan, malformed projection, prompt drift, and source drift block before authority writes |
| `n6_loopback_triage` | implemented L1/L2 | optional support-only runtime/admission slot; binds failed N6 draft identity and frozen N5->N6 lineage; malformed present triage blocks; absent triage remains deterministic fallback; cannot create candidate, N5 selection, handoff, or downstream recheck authority |
...
| v1b N6 | `topic-selection.v1b.generate-topic-question-candidates.v1` | `n6_question_candidate_draft` | frozen semantic artifact via `WorkflowHarness`; provider/codex generation currently external harness/script | model draft before deterministic candidate gate | `v1b_n6_topic_question_generation`; owner TBD v1b semantic-support adapter | `topic-selection.v1b.topic-question-candidates.single-agent.v1`; `TopicQuestionCandidateSetDraft@v1` |
