...
- Current phase: T-107 exit gate accepted for v1b product-level WorkflowHarness normalization and supplemented with harness-native HTTP invocation capability, a standalone harness e2e runner, provider-backed standalone canary mode, and external bundled-Codex CLI N4/N6/N8 variance probes. v1b now has executable N1 intake snapshot, N2 constraint profile, N3 intake readiness, N4 research-slice option generation, N5 research-slice selection, N6 topic-question candidate generation, N7 topic-question contract materialization/trial coordination, N8 topic value assessment, N9 value disposition, N10 draft topic package creation, and N11 v1c input bundle publication runners behind the WorkflowHarness admission/replay shell. Local/Codex-assisted automation invokes the full N1-N11 shared harness chain through `/topic-selection/v1b/workflow-harness/nodes/:nodeId/invocations` and harness artifact routes, either from tests or `pnpm topic-selection:v1b-harness-e2e`. Provider-backed runner mode now exercises N4/N6/N8 live semantic drafts through the registry-backed LLM gateway; DashScope repeat=3 and OpenAI repeat=3 both passed as acceptance canaries. Legacy v1b HTTP orchestration write routes have been physically removed; old route strings remain only as negative 404 assertions.
...
- [x] v1b WorkflowHarness scenarios cover happy path, negative gates, loopbacks, replay/idempotency, provider/Codex admission canaries, full harness-native HTTP N1-N11 invocation for local automation, a standalone harness e2e runner, provider-backed N4/N6/N8 semantic canaries, DashScope repeat=3 provider execution, OpenAI repeat=3 provider execution, external bundled-Codex CLI N4/N6/N8 variance, and a Prisma-backed harness HTTP route smoke. Full multi-turn human-interactive Codex-session variance and broader multi-sample operational stability remain outside this service-level T-107 slice.
