...
This contract defines what the later experiment-foundation desktop smoke must prove. It is intentionally implementation-neutral: the renderer consumes T-076/T-077 APIs and displays state, but it MUST NOT generate readiness, promotion, materialization, execution, validation, or evidence decisions.
...
- Out of scope for this contract: new shared contracts, Prisma schema changes, backend route changes, real cloud canary execution, and renderer-owned domain payload generation.
...
| 6 | Recipe/Materialization | Inspect or upsert frozen `recipe_draft`, `run_recipe`, `version_lock`, `training_task_spec`, and `training_task_materialization_result` payloads. | Registry create/upsert/get/list calls only. | UI labels these as frozen contracts; it does not generate a `RunRecipe` or materialize a task spec. |
...
| Idempotency conflict | `VERSION_CONFLICT`. | Show existing idempotency key context if backend returns it; do not auto-submit with a new key. |
...
When UI automation is scheduled, it SHOULD produce redacted artifacts under:
...
Phase 4 is satisfied when this contract is reviewed and later UI automation can be implemented directly from it without redefining backend semantics.
